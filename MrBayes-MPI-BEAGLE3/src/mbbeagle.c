...
#   endif
        ...
        // 获取可用资源列表以确定是否只有一个CPU资源
        BeagleResourceList* resourceList = beagleGetResourceList();
        int cpuResourceCount = 0;
        
        // 计算CPU资源数量
        for (int i = 0; i < resourceList->length; i++) {
            if (resourceList->list[i].supportFlags & BEAGLE_FLAG_FRAMEWORK_CPU) {
                cpuResourceCount++;
            }
        }
        
        // 如果只有一个CPU资源，跳过基准测试直接使用资源0
        if (cpuResourceCount == 1 && resourceList->length == 1) {
            resource = 0;  // 使用第一个资源(CPU)
            resourceCount = 1;
            MrBayesPrint ("\n%s   Single CPU resource detected, skipping benchmark... ", spacer);
        } else {
...
MrBayesPrint ("%s      MODEL STATES: %d", spacer, numModelStates);
    ...
    // beagleThreadCount = 1;
    // MrBayesPrint("This is a Windows modification of MrBayes-BEAGLE and does not support BEAGLE's multi-threading.\nThus, beagleThreadCount is forced to be 1.\n\n");
